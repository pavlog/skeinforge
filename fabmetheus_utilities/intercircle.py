...
def getAroundsFromLoop(loop, radius, thresholdRatio=0.9,innerMagic=1.0):
	'Get the arounds from the loop.'
	return getAroundsFromPoints(getPointsFromLoop(loop, radius, thresholdRatio), radius,innerMagic)
...
def getAroundsFromPoints( points, radius,innerMagic=1 ):
	...
	for center in centers:
		...
		if isLargeSameDirection(inset, center, radius*innerMagic):
...
def getInsetLoopsFromLoops(loops, radius,outsetInnerMagic=1.0):
	'Get the inset loops, which might overlap.'
	insetLoops = []
	for loop in loops:
		insetLoops += getInsetLoopsFromLoop(loop, radius,0.9,outsetInnerMagic)
	return insetLoops
